...
class dAuthComponent extends Object
{
	...
	var $pages =		array(	'default' => '/',
								'login_success' => '/root/branches/',
								'logout_success' => '/users/login/',
								'logout_failure' => '/',
								'login' => '/users/login',
								'logout' => '/users/logout',
	...
	var $messages =		array('credentials_mismatch' => 'Zadané jméno/heslo není platné.',
								'bad_username' => 'Zvolte si prosím jiné/lepší jméno',
								'registration_success' => 'Registrace proběhla v pořádku.',
	...
	function attemptRegister($postUser = null,$ip = null)
	{
		...
		{
			if(!isset($postUser['User']['username']) || !$postUser['User']['username'] || $this->controller->User->find('count', array('conditions'=>array('username'=>$postUser['User']['username']))))
			{
			...
			else
			{
            	...
            	{
            		$success = true;
            		//$this->controller->Session->setFlash($this->messages['registration_success'], null);
            	}
            	else
            	{
